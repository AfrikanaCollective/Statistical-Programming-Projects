## CIN-Neonatal Documentation Scores daisy 
...
if(!file.exists("Documentation_Score.rds")){
  ...
  Maternal <- Dataset %>%
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(
      `Maternal History` = getDocumentationScore(dplyr::across(Maternal.Age:Maternal.Parity)),
      `Denom.Maternal History` = length(dplyr::across(Maternal.Age:Maternal.Parity))
    ) %>%
    dplyr::select(
      key,
      `Maternal History`,
  ...
  Complaints <- Dataset %>%
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(
      `Presenting Complaints` = getDocumentationScore(dplyr::across(Temparature:Apnoea)),
      `Denom.Presenting Complaints` = length(dplyr::across(Temparature:Apnoea))
    ) %>%
    dplyr::select(
      key,
  ...
  Signs <- Dataset %>%
    ...
    dplyr::rowwise() %>%
    dplyr::mutate(
      `Cardinal Signs` = getDocumentationScore(dplyr::across(Grunting:Floppy)),
      `Denom.Cardinal Signs` = length(dplyr::across(Grunting:Floppy))
    ) %>%
    dplyr::select(
      key,
      `Cardinal Signs`,
       `Denom.Cardinal Signs`
    )
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.pos='H', comment = NA}
plot.cardinal.signs <- Validate_Scores %>%
  dplyr::filter(!(Hospital %in% c("Bondo DH", "Karatina District Hospital"))) %>%
  dplyr::filter(!(Admission.DateTime < as.Date("2018-01-01", formart = "%Y-%m-%d"))) %>%
  dplyr::mutate(
    Month.Year = lubridate::as_date(zoo::as.yearqtr(Admission.DateTime))
  ...
  dplyr::group_by(Hospital, Month.Year) %>%
  dplyr::summarise(
    `Score` = sum( `Cardinal Signs`, na.rm = T),
    `Denominator` = sum(`Denom.Cardinal Signs`, na.rm = T),
    `Rate` = round(Score/Denominator,4)) %>%
  dplyr::ungroup() 

plot.cardinal.signs %>% ggplot() +
  geom_point(aes(x = Month.Year, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Month.Year, y = Rate), data = plot.cardinal.signs, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0.,1)) +
  scale_x_date(date_labels = "%B\n%Y",
               breaks = function(x) seq.Date(from = min(x), to = max(x), by = "6 months"),
               minor_breaks = function(x) seq.Date(from = min(x), to = max(x), by = "3 months"),
  ...
  labs(x= "Calendar Date", 
       y= "Percentage Documentation", 
       title= "Cardinal signs on examination ",
       caption = "Each unique colour-coded point represents an individual hospital") +
  theme(legend.position = "none")+
  theme(plot.title = element_text(hjust = 0.5))
