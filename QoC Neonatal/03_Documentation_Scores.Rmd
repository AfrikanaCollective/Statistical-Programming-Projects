...
```{r echo=FALSE, warning=FALSE, message=FALSE, comment = NA}
  ...
  # Analysis.Dataset.History - Original dataset you are using
# Hospital - Hospital variable
# Admission_Date - The date of admission per variable

TimerTable <- Dataset %>%
  dplyr::select(Hospital, Admission_Date) %>%
  dplyr::mutate(
    Report_Month = as.Date(zoo::as.yearmon(Admission_Date)),
    Counter = 1
  ) %>%
  dplyr::select(-Admission_Date) %>%
  unique() %>%
  dplyr::filter(
    Report_Month != as.Date("2020-12-01", format = "%Y-%m-%d")
  ) %>%
  dplyr::group_by(Hospital) %>%
  dplyr::mutate(Time = order_by(Report_Month,cumsum(Counter))) %>%
  dplyr::select(-Counter) %>%
  dplyr::arrange(Hospital, Report_Month, Time)

# Combine to original dataset

Update.Dataset <- Dataset %>%
  dplyr::mutate(Report_Month = as.Date(zoo::as.yearmon(Admission_Date))) %>%
  merge(., TimerTable, by = c("Hospital", "Report_Month"))
        
# The variable of interest is Update.Dataset$Time
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.pos='H', comment = NA}
Validate_Scores <- Update.Dataset %>%
  dplyr::select(
    key,
    Hospital,
    Male,
...
plot.Demographics %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.Demographics, method = "loess", formula = 'y ~ x') +
 ...
 #              minor_breaks = function(x) seq.Date(from #= min(x), to = max(x), by = "3 months"),
  ...
  demog<-c("Male",
                               ...
                               "Gestational age",
                               "Birth weight")
knitr::kable(demog,caption="Variables",col.names = NULL)%>%
...
plot.admission.information %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.admission.information, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0.,1)) +
 # scale_x_date(date_labels = "%b\n%Y",
  #             breaks = function(x) seq.Date(from = #min(x), to = max(x), by = "6 months"),
 #              minor_breaks = function(x) seq.Date(from #= min(x), to = max(x), by = "3 months"),
  ...
  theme(legend.position = "none")+
  theme(plot.title = element_text(hjust = 0.5))
  admissions.list<-c("NAR use recorded",
      "Birth date and time",
      ...
      "Admission diagnosis count")
 knitr::kable(admissions.list,caption="Variables",col.names =NULL)%>%
    kable_styling(bootstrap_options = "striped")
...
plot.discharge.information %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.discharge.information, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0.,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
             #  breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
              # minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
  ...
  theme(legend.position = "none")+
  theme(plot.title = element_text(hjust = 0.5))
  discharge.list<-c("Discharge date and time",
      "Discharge weight",
      "Outcome",
     " Discharge diagnosis count")
      
 knitr::kable(discharge.list,caption="Variables",col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
...
plot.Maternal.history %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.Maternal.history, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
             # breaks = function(x) seq.Date(from = min(x), to = max(x), #by = "6 months"),
            #   minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
 ...
 knitr::kable(maternal.list,caption="Variables",col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
...
plot.presenting.complains %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.presenting.complains, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
 # scale_x_date(date_labels = "%b\n%Y",
              # breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
              # minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
  ...
  theme(legend.position = "none")+
  theme(plot.title = element_text(hjust = 0.5))
  complaints.cardinal.list<-c("Temparature",
      "Fever",
     " Convulsions",
      "Difficulty breathing",
      "Difficulty feeding",
     ...
     " Bulging fontanelle",
      "Floppy")
   knitr::kable(complaints.cardinal.list,caption="Variables", col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
...
plot.other.signs %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.other.signs, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0.,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
             #  breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
               ##minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
  ...
  theme(legend.position = "none")+
  theme(plot.title = element_text(hjust = 0.5))
other.signs.list<-c("Can Suck or Breastfeed",
      "Bilateral air entry",
      "Crackles",
      "Indrawing",
      "Capillary refill",
      ...
      "Irritable",
      "Jaundice",
      "Glucose test ordered")
   knitr::kable(other.signs.list,caption="Variables",col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
...
plot.treatment %>% ggplot() +
  geom_point(aes(x =Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data = plot.treatment, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
             #  breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
               #minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
...
treatment.list<-c( "Age",
  ...
  knitr::kable(treatment.list,caption="Variables",col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
```
#Dosage was calculated using the formula Dosage = dose prescribed/weight
#Penicilin Under dose -  a prescription of less than 40,000iu/kg
#Penicillin Overdose - a prescription >60,000 iu/kg 
#Penicillin Correct dose -  a prescription >=40000 & <=60000 iu/kg

#Gentamicin Underdose - a prescription <2.4mg/kg for weight below 2kg or <4mg/kg for weight >=2kg.
#Gentamicin Overdose - a prescription >3.6mg/kg for weight below 2kg or <6mg/kg for weight >=2kg.
...
plot.fluids %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data =plot.fluids, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
              # breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
              # minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
  ...
  fluids.list<-c("Age",
      ...
      "Frequency of adminstration",
      "Route of adminstration")
      knitr::kable(fluids.list,caption="Variables",col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
```
...
plot.feeds %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data =plot.feeds, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
               #breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
               #minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
...
plot.monitoring %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data =plot.monitoring, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
              # breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
              # minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
  ...
  theme(legend.position = "none")+
  theme(plot.title = element_text(hjust = 0.5))
Vitals.list<-c("Weight monitoring recorded",
      "Number of times weight is monitored",
    "Pulse rate recorded",
      "Tempereture recorded",
      "Pulse oximetry recorded",
      "Respiratory rate recorded")
      knitr::kable(Vitals.list,caption="Variables",col.names = NULL)%>%
    kable_styling(bootstrap_options = "striped")
```
...
plot.supportive.care %>% ggplot() +
  geom_point(aes(x = Time, y = Rate, group = Hospital, color = Hospital)) +
  ggsci::scale_color_d3(palette = "category20b") + 
  geom_smooth(aes(x = Time, y = Rate), data =plot.supportive.care, method = "loess", formula = 'y ~ x') +
  scale_y_continuous(labels = scales::percent, breaks =seq(0,1,.2), limits =c(0,1)) +
  #scale_x_date(date_labels = "%b\n%Y",
   #            breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "6 months"),
             #  minor_breaks = function(x) seq.Date(from = min(x), to = #max(x), by = "3 months"),
